...
#===============================================================================
import os
scriptVersionString = "SemiExtendedCorr.py $Revision: 1.0 $"
...
#-------------------------------------------------------------------------------
# Convolve two radial functions
def RFT(rad,f1,f2):
	"""
	========================================================================
	Calculate the convolution of two radial functions

	Inputs:
	  rad: (array float) radius vector
	  f1:  (array float) first radial function, corresponding to rad
	  f2:  (array float) second radial funcion, corresponding to rad
	"""
...
def spireMonoBeamConvGauss(freqx,beamRad,beamProfs,beamConst,srcWidth,effFreq,gamma,array):
	"""
	========================================================================
	...
	Inputs:
	  ...
	  srcWidth:  (float) source Gaussian width (in arcsec)
	...
	Calculation:
	  ...
	  Convolves with a source profile
...
def spireMonoAreas(freq,beamProfs,effFreq,srcWidtrh,gamma,array,freqFact=100):
	...
	for fx in range(nNuArea):
		...
		beamMonoAreaSparse[fx]=spireMonoBeamConvGauss(freq[f],beamRad,beamProfs,beamConst,srcWidth,effFreq,gamma,array)[0]
...
def calcBeamMonoAreaConvGauss(srcWidth):
    ...
    for band in spireBands:
    	...
    	beamMonoArea[band] = spireMonoAreasConvGauss(freq, beamProfs, 
    	  srcWidth,spireEffFreq[band], gamma, band)
    return beamMonoArea
...
def calcKBeam(alphaK):
    ...
    beamPip = calcOmegaEff(alphaK)
    for band in spireBands:
    	#pipeline beam areas
    	kBeam[band] = beamPip0[band]/beamPip[band]
...
def calcKBeam_BB(betaK, tempK):
    ...
    kBeam = {'PSW': Double.NaN, 'PMW': Double.NaN, 'PLW': Double.NaN}
    beamPip0 = calcOmegaEff(-1.0)
    beamPip = calcOmegaEff_BB(betaK,tempK)
    for band in spireBands:
    	#pipeline beam areas
    	kBeam[band] = beamPip0[band]/beamPip[band]
